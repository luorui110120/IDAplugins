...
BADDATA:
  ...
  uchar *packed = qalloc_array<uchar>(size);
  ...
  for ( int i=0; i<3; i++ )
  {
    ...
    if ( size > 0 )
    {
      packed = unpack_rle(packed, size, cea, fpos+uint32(packed-packed_sav));
      if ( packed == NULL )
        return 0;
    }
    if ( usize < cea-ea )
...
void idaapi load_file(linput_t *li, ushort neflags, const char * fileformatname)
{
  ...
  ResourceMapEntry *re = qalloc_array<ResourceMapEntry>(h.numRecords);
  if ( re == NULL )
